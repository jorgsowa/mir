...
- **Class ancestry resolution:** removed the legacy short-name
  `class_ancestors` query path in favor of canonical-FQCN ancestry lookups.
- **Release version sync:** bumped the workspace and internal crate dependency
...
- **Incremental cache invalidation:** mirror workspace growth now invalidates
  ...
  preventing stale analyzer state after new declarations become visible.
- **Analyzer lint hygiene:** simplified the session ingest cache-invalidation
  types to satisfy Clippy's type-complexity checks.
...
- **Symfony salsa query regression coverage:** CI can now download the
  Symfony fixture and exercise analyzer queries against a full framework
  corpus, covering symbol indexes, declarations, definitions, inheritance,
...
- **Member-name-only gate for static-only member references:**
  ...
  method resolves to a static-only declaration and, in that case, gates
  never-committed files on the member name alone — dropping the owner short
  name the general gate ORs in, which on common owner names (Color, Asset,
  ...) admits most of a large workspace for analysis. Sound because every
  posting-producing static reference spells the member token — `Owner::m()`,
  inherited `Sub::m()`, `self::`/`static::`/`parent::m()`, aliased
  `Alias::m()`, instance-receiver `$obj::m()`, callable strings `'Owner::m'`
  — while dynamic member names (`Owner::$m()`) produce no posting at all.
  (An earlier unreleased design gated on member AND owner-or-subtype name
  instead; it was scrapped for dropping `$obj::m()` sites whose file never
...
- **Constructor gate admits explicit re-init call sites:** the
  ...
  calls living in files that never name the class — e.g. a receiver typed
...
- **`is_a()`/`is_subclass_of()` didn't narrow property-access receivers:** both checks only handled a plain variable receiver (`extract_var_name`), unlike plain `instanceof`'s `narrow_prop_instanceof` — `is_a($this->item, Foo::class)` and `is_subclass_of($this->item, Foo::class)` silently no-op'd on a property receiver, missing real `UndefinedMethod`/`PossiblyNullMethodCall` bugs after a proving guard. Adds `narrow_prop_is_a`/`narrow_prop_is_subclass_of` mirroring the existing variable-based semantics, sharing a new `apply_prop_narrowed` helper with `narrow_prop_instanceof`.
