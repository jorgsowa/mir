...
//!
//! `lazy_load.rs` exercises the *batch* path (`analyze_paths`). This file
...
fn analyze_open_file_unindexed(
    ...
    let root = crate::common::create_temp_dir("priority_index_unindexed");
    for (name, src) in lib_files {
...
fn priority_index_pre_loads_docblock_only_param_class() {
    let root = crate::common::create_temp_dir("docblock_preload");
    fs::create_dir_all(root.path().join("src")).unwrap();
...
fn analyze_open_file_unindexed_psr0(
    ...
    let root = crate::common::create_temp_dir("priority_index_unindexed_psr0");
...
fn session_diagnostics_lazy_loads_psr4_class_without_deadlock() {
    let root = crate::common::create_temp_dir("session_diagnostics_psr4");
    fs::create_dir_all(root.path().join("src")).unwrap();
    fs::write(
