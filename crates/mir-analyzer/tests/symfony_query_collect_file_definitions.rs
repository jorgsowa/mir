...
fn symfony_query_collect_file_definitions() {
    ...
    assert_eq!(defs.slice.classes.len(), 1, "RequestStack.php should define exactly one class");
    assert!(
        defs.slice.interfaces.is_empty(),
        "RequestStack.php should not define interfaces"
    );
    assert!(defs.slice.traits.is_empty(), "RequestStack.php should not define traits");
    assert!(defs.slice.enums.is_empty(), "RequestStack.php should not define enums");
    assert!(
    ...
    assert_eq!(class.parent, None, "RequestStack should not extend another class");
    assert!(
        class.interfaces.is_empty(),
        "RequestStack should not declare implemented interfaces"
    );
    assert!(class.traits.is_empty(), "RequestStack should not use traits");
    ...
    assert!(!class.is_internal, "RequestStack should be a userland Symfony class");
    assert!(
    ...
    assert!(!push.is_constructor, "push should not be marked as constructor");
    assert_eq!(push.params.len(), 1, "push should take a single Request argument");
    assert!(push.return_type.is_some(), "push should preserve its native void return type");
    ...
    assert!(!pop.is_constructor, "pop should not be marked as constructor");
    assert!(
        pop.params.is_empty(),
        "pop should not take any parameters"
    );
    assert!(
