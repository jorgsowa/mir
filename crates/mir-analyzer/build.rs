...
fn main() {
    ...
    generate_custom_stub_files(Path::new(&manifest_dir), Path::new(&out_dir));
...
/// built-in function names for the extensions listed in `STUB_DIRS`.
fn generate_builtin_fn_names(manifest_dir: &Path, out_dir: &Path) {
    ...
    // Build lowercase set of included stub directory names for O(1) lookup.
    let stub_dirs_lower: std::collections::HashSet<String> =
        STUB_DIRS.iter().map(|d| d.to_lowercase()).collect();
...
// ---------------------------------------------------------------------------
// Custom stub embedding — stubs/{ext}/*.php
// ---------------------------------------------------------------------------

/// Walk every `stubs/{ext}/` directory and embed each `.php` file as a
/// `(workspace-relative-path, content)` pair in `CUSTOM_STUB_FILES`.
///
/// Paths use the workspace root as the prefix so they look like
/// `"stubs/standard/basic.php"` — stable virtual identifiers for go-to-definition.
fn generate_custom_stub_files(manifest_dir: &Path, out_dir: &Path) {
    let workspace_root = find_workspace_root(manifest_dir);
    let stubs_dir = workspace_root.join("stubs");
    let out_path = out_dir.join("custom_stub_files.rs");

    if !stubs_dir.exists() {
        fs::write(
            &out_path,
            "/// No stubs/ directory found.\n\
             pub(crate) static CUSTOM_STUB_FILES: &[(&str, &str)] = &[];\n",
        )
        .unwrap();
        return;
    }
    ...
    let mut code = String::from(
        "/// Custom PHP stubs embedded from stubs/ — precise type overrides.\n\
         /// Auto-generated by build.rs — do not edit directly.\n\
         pub(crate) static CUSTOM_STUB_FILES: &[(&str, &str)] = &[\n",
    );
