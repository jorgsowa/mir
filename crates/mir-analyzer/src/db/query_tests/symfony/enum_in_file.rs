...
use super::{fqcn, load_full_symfony_fixture};
use crate::db::{enum_in_file, MirDatabase};
...
fn symfony_query_enum_in_file() {
    ...
    assert!(
        found.scalar_type.is_none(),
        "TruncateMode should be a pure enum"
    );
    assert_eq!(
        found
            .interfaces
    ...
    assert!(
        ...
        "TruncateMode should not use traits"
    );
    assert!(
        found.own_methods.is_empty(),
        "TruncateMode should not declare methods"
    );
    assert!(
        found.own_constants.is_empty(),
        "TruncateMode should not declare extra constants"
    );
