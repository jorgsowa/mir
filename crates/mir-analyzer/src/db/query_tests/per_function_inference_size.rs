...
//!
//! Run with:
//!   cargo test -p mir-analyzer --lib --release per_function_inference_size -- --nocapture --ignored

use std::path::PathBuf;
use std::sync::Arc;

use crate::db::MirDatabase;
use crate::{discover_files, perf_fixture::PerfFixture, AnalysisSession, BatchOptions, PhpVersion};
...
fn measure_per_function_inference_size() {
    ...
    let n_functions = crate::db::workspace_functions(&db).len();
    let class_fqcns = crate::db::workspace_classes(&db);
    ...
    let sizeof_refloc = std::mem::size_of::<crate::db::RefLoc>();
