    ...
    match &class {
        ClassLike::Interface(iface) => {
            for e in iface.extends.iter() {
                ...
                let parent_fqcn = Fqcn::interned(db, Name::new(e.as_ref()));
        ...
        ClassLike::Class(cls) => {
            if let Some(ref p) = cls.parent {
                ...
                let parent_fqcn = Fqcn::interned(db, Name::new(p.as_ref()));
                for a in &class_ancestors(db, parent_fqcn).0 {
                    add(a, &mut all, &mut seen);
                }
            }
            for iface in cls.interfaces.iter() {
                ...
                let iface_fqcn = Fqcn::interned(db, Name::new(iface.as_ref()));
