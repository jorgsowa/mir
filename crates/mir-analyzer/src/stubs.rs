...
use mir_types::{Atomic, PhpVersion, Union};
...
pub fn load_stubs(codebase: &Codebase, version: PhpVersion) {
    // Layer 1: parse phpstorm-stubs for comprehensive built-in coverage.
    load_phpstorm_stubs(codebase);
    // Layer 1b: remove functions that do not exist in the target version.
    // phpstorm-stubs contain all historical PHP symbols; we prune the ones
    // that were removed before or at the target version.
    remove_version_gated_functions(codebase, version);
    // Layer 2: hand-written stubs override/supplement where needed.
    load_functions(codebase, version);
    load_classes(codebase);
    load_interfaces(codebase);
}

/// Remove functions from the codebase that were removed in a PHP version ≤ `target`.
///
/// phpstorm-stubs include all historical PHP symbols. This pass deletes the
/// functions whose removal version is at or before the target, ensuring that
/// code targeting PHP 8.0+ does not see functions removed in 8.0.
fn remove_version_gated_functions(codebase: &Codebase, version: PhpVersion) {
    // Functions removed in PHP 8.0
    if version >= PhpVersion::PHP_80 {
        for name in &[
            "each",
            "create_function",
            "money_format",
            ...
            codebase.functions.remove(*name);
        }
    }
    // Functions removed in PHP 8.1
    if version >= PhpVersion::PHP_81 {
        for name in &[
            "mhash",
            "mhash_count",
...
pub fn is_builtin_function(name: &str) -> bool {
    static BUILTIN_FUNCTIONS: LazyLock<HashSet<Box<str>>> = LazyLock::new(|| {
        ...
        load_stubs(&codebase, PhpVersion::PHP_80);
        codebase
            .functions
            .iter()
...
/// Register a function only within the version range `since <= version < until`.
fn reg_range(
    codebase: &Codebase,
    version: PhpVersion,
    since: PhpVersion,
...
fn load_functions(codebase: &Codebase, version: PhpVersion) {
    // ---- String ----------------------------------------------------------------
    ...
    // ---- Functions removed in PHP 8.0 (available only when target < 8.0) -----

    let v = version;
    let php74 = PhpVersion::PHP_74;
    let php80 = PhpVersion::PHP_80;
    let php81 = PhpVersion::PHP_81;

    // each() — removed in PHP 8.0; takes array by reference, returns array|false
    reg_range(
        codebase,
        v,
        php74,
    ...
    reg_range(
        ...
        php80,
        "money_format",
        vec![req("format"), req("number")],
        t_str(),
    );
    // convert_cyr_string() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
        php80,
        "convert_cyr_string",
        vec![req("str"), req("from"), req("to")],
        t_str(),
    );
    // hebrevc() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
        php80,
        "hebrevc",
        vec![req("string"), opt("max_chars_per_line")],
        t_str(),
    );
    // hebrev() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
    ...
    // get_magic_quotes_runtime() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
    ...
    // restore_include_path() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
    ...
    // define_syslog_variables() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
    ...
    reg_range(
        ...
        php80,
        "fgetss",
        vec![req("handle"), opt("length"), opt("tags")],
        t_str_or_false(),
    );
    // gzgetss() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
        php80,
        "gzgetss",
        vec![req("zp"), req("length"), opt("allowable_tags")],
        t_str_or_false(),
    );
    // image2wbmp() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
        php80,
        "image2wbmp",
        vec![req("image"), opt("filename"), opt("foreground")],
        t_bool(),
    );
    // jpeg2wbmp() — removed in PHP 8.0
    reg_range(
        codebase,
        v,
        php74,
    ...
    // ---- Functions removed in PHP 8.1 (mhash family) -------------------------

    // mhash() — removed in PHP 8.1
    reg_range(
        codebase,
        v,
        php74,
        php81,
        "mhash",
        vec![req("algo"), req("data"), opt("key")],
        t_str_or_false(),
    );
    // mhash_count() — removed in PHP 8.1
    reg_range(codebase, v, php74, php81, "mhash_count", vec![], t_int());
    // mhash_get_block_size() — removed in PHP 8.1
    reg_range(
        codebase,
        v,
        php74,
    ...
    // mhash_get_hash_name() — removed in PHP 8.1
    reg_range(
        codebase,
        v,
        php74,
    ...
    // mhash_keygen_s2k() — removed in PHP 8.1
    reg_range(
        codebase,
        v,
        php74,
        php81,
        "mhash_keygen_s2k",
        vec![req("algo"), req("password"), req("salt"), req("bytes")],
        t_str_or_false(),
    );
}
...
mod tests {
    ...
    fn stubs_codebase() -> Codebase {
        let cb = Codebase::new();
        load_stubs(&cb, PhpVersion::PHP_80);
        cb
    }
    ...
    #[test]
    fn php80_removed_functions_absent_on_php80_target() {
        let cb = Codebase::new();
        load_stubs(&cb, PhpVersion::PHP_80);
        // These were removed in 8.0, so they must NOT be visible on an 8.0 target.
        for name in &[
            "each",
            "create_function",
            "money_format",
            ...
            assert!(
                !cb.functions.contains_key(*name),
                "`{name}` must not be registered for PHP 8.0 target"
            );
        }
    }

    #[test]
    fn php80_removed_functions_present_on_php74_target() {
        let cb = Codebase::new();
        load_stubs(&cb, PhpVersion::PHP_74);
        // These were removed in 8.0, so they MUST be visible on a 7.4 target.
        for name in &[
            "each",
            "create_function",
            "money_format",
            ...
            assert!(
                cb.functions.contains_key(*name),
                "`{name}` must be registered for PHP 7.4 target"
            );
        }
    }

    #[test]
    fn php81_removed_mhash_absent_on_php81_target() {
        let cb = Codebase::new();
        load_stubs(&cb, PhpVersion::PHP_81);
        for name in &[
            "mhash",
            "mhash_count",
            "mhash_get_block_size",
            ...
            assert!(
                !cb.functions.contains_key(*name),
                "`{name}` must not be registered for PHP 8.1 target"
            );
        }
    }

    #[test]
    fn php81_removed_mhash_present_on_php74_target() {
        let cb = Codebase::new();
        load_stubs(&cb, PhpVersion::PHP_74);
        for name in &[
            "mhash",
    ...
    #[test]
    fn each_param_is_byref() {
        let cb = Codebase::new();
        load_stubs(&cb, PhpVersion::PHP_74);
        let func = cb
            .functions
            .get("each")
            .expect("each must be registered on PHP 7.4");
        let param = func.params.first().expect("each must have one parameter");
        assert!(param.is_byref, "each's array param must be by-reference");
    }
}
