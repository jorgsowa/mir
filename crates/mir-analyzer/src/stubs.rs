...
#[allow(dead_code)]
pub(crate) fn load_stubs(db: &mut MirDbStorage) {
...
pub(crate) fn stub_slice_from_source(
    ...
    let result = php_rs_parser::parse(content);
...
/// matched against `MirDbStorage`'s registered `SourceFile`s when stubs are
...
mod tests {
    use super::*;
    use crate::db::{class_exists, constant_exists, function_exists, MirDatabase, MirDbStorage};

    fn stubs_codebase() -> MirDbStorage {
    ...
    fn assert_cls(cb: &MirDbStorage, name: &str) {
        assert!(
            class_exists(cb, name),
            "expected stub class `{name}` to be registered"
        );
    }

    fn assert_iface(cb: &MirDbStorage, name: &str) {
        assert!(
            class_exists(cb, name),
    ...
    fn assert_const(cb: &MirDbStorage, name: &str) {
        assert!(
    ...
    fn class_lookup_is_case_insensitive() {
        ...
        let cb = stubs_codebase();
        assert!(class_exists(&cb, "ArrayObject"));
        assert!(class_exists(&cb, "arrayobject"));
        assert!(class_exists(&cb, "ARRAYOBJECT"));
        assert!(class_exists(&cb, "ArrayOBJECT"));
    }
